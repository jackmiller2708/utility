...
export class ArtifactObjectUrlService {
  ...
  private resolve(id: string, kind: UrlKind): string | null {
    const cache = kind === 'file' ? this._fileUrls : this._downloadUrls;
    ...
    if (!this._pending.has(pendingKey)) {
      ...
      const blob$ = kind === 'file' ? this._apiClient.getArtifactFileBlob$(id) : this._apiClient.getArtifactDownloadBlob$(id);
